...
import { sanitizeName } from '@/lib/jnu-abc';
import { escapeDoubleQuotes } from '@/lib/jnu-doc/html';
    ...
    replacement: (content: string, node: any): string => {
      ...
      if (node.nodeName === 'PRE') {
        ...
        const preservedContent = codeContent
          .split('\n')
          .map((line: string) => line.trimEnd()) // 줄 끝의 공백만 제거
          .join('\n')
          .trim();
...
// * frontmatter 생성
const mdFrontmatter = (properties: Record<string, any>) => {
  let frontmatter = '---\n';
